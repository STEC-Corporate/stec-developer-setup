#!/usr/bin/env bash
# LLM Advisor — detecta domínio da sessão e recomenda modelo ideal
# Acionado pelo hook Stop do Claude Code via settings.json
...
# Captura contexto recente da sessão
RECENT=""
if [[ -f "${CLAUDE_TRANSCRIPT_PATH:-}" ]]; then
    RECENT=$(tail -c 3000 "$CLAUDE_TRANSCRIPT_PATH" 2>/dev/null || true)
fi
[[ -z "$RECENT" ]] && RECENT=$(cat 2>/dev/null || true)
[[ -z "$RECENT" ]] && exit 0
...
MSG="💡 LLM Advisor [$DOMAIN]: $MODEL recomendado"
